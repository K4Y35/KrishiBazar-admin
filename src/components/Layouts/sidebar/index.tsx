...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={`fixed left-0 top-0 z-50 flex h-screen w-72.5 flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
            ...
            <ul className="mb-6 flex flex-col gap-1.5">
              ...
              {menuItems.map((menuItem: MenuItem, key: number) => {
                ...
                const isActive = menuItem.submenu 
                  ? menuItem.submenuItems?.some(item => pathname === item.path) || false
                                    ...
                                    <li key={index}>
                                      <Link
                                        ...
                                        className={`group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-white ${
                                          pathname === submenuItem.path ? '!text-white' : ''
                                        }`}
                                      >
                                        {submenuItem.title}
                                      </Link>
